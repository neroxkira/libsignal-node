...
class SessionCipher {
    ...
    async decryptWithSessions(data, sessions) {
        /* Iterate through available sessions and attempt decryption. 
    ...
    async decryptWhisperMessage(data) {
        ...
        return await this.queueJob(async () => {
            ...
            if (record.isClosed(result.session)) {
                /* Session is closed, but message might be from a backlog. 
                   Process normally but track session status. */
            }
            await this.storeRecord(record);
    ...
    async doDecryptWhisperMessage(messageBuffer, session) {
        ...
        if (!chain.messageKeys.hasOwnProperty(message.counter)) {
            /* Handled already decrypted message or missing key */
        ...
        /* Verification failure here usually indicates session mismatch rather than corruption */
    ...
    maybeStepRatchet(session, remoteKey, previousCounter) {
        ...
        /* Swap the ephemeral key and calculate new sending chain */
